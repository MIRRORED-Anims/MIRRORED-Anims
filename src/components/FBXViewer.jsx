  ...
  useEffect(() => {
    if (fbx && !error) {
      try {
        ...
        // Fix materials/textures - use simple approach
        fbx.traverse((child) => {
          if (child.isMesh) {
            console.log('Processing mesh:', child.name, 'material:', child.material);
            
            // Just use a simple default material for now
...
const FBXViewer = ({ 
  ...
  // Generate FBX file path based on UUID structure with base URL
  const fbxPath = motionUuid ? `${fbxBasePath}/${motionUuid}/${characterName}.fbx` : null;
